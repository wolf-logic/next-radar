...
export function renderBlipDescription(blip, ring, quadrant, tip, groupBlipTooltipText, userSlug, radarSlug) {
  ...
  if (!groupBlipTooltipText) {
    ...
    blipItemDiv
      .append("div")
      .classed("blip-list__item-container__description markdown-content", true)
...
export function renderQuadrantTables(quadrants, rings) {
  ...
  quadrants.forEach(function (quadrant) {
    ...
    // Get unique rings from blips
    const uniqueRings = Array.from(
      new Set(
        quadrant.quadrant
          .blips()
          .map(blip => blip.ring())
          .map(ring => JSON.stringify({ name: ring.name(), order: ring.order() }))
      )
    ).map(ringStr => JSON.parse(ringStr));
    
    // Sort rings by their order property to ensure consistent ordering
    const sortedRings = uniqueRings.sort((a, b) => a.order - b.order);
    ...
    sortedRings.forEach(function (ring) {
      ...
      quadrantContainer
        ...
        .attr("data-ring-order", ring.order);
    });
  });
}
