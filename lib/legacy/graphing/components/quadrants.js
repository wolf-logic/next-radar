...
import { getElementWidth, getElementHeight, decodeHTML } from "@/lib/legacy/util/htmlUtil";
import { toRadian } from "@/lib/legacy/util/mathUtils";
import { getRingIdString } from "@/lib/legacy/util/stringUtil";
import { isHandlingPopState, updateURL } from "./quadrantSubnav";
import {
    graphConfig,
    getGraphSize,
...
function onQuadrantClick(order, startAngle, name) {
    if (isHandlingPopState) return;
    updateURL(name);
    selectRadarQuadrant(order, startAngle, name);
}

export function selectRadarQuadrant(order, startAngle, name) {
...
export function renderRadarQuadrants(size, svg, quadrant, rings, ringCalculator, tip) {
    const quadrantGroup = svg
        .append("g")
        .attr("class", "quadrant-group quadrant-group-" + quadrant.order)
        .on("mouseover", mouseoverQuadrant.bind({}, quadrant.order))
        .on("mouseout", mouseoutQuadrant.bind({}, quadrant.order))
        .on("click", () => onQuadrantClick(quadrant.order, quadrant.startAngle, quadrant.quadrant.name()))
        .on("keydown", function (e) {
            if (e.key === "Enter") {
                onQuadrantClick(quadrant.order, quadrant.startAngle, quadrant.quadrant.name());
            }
        });
...
export function renderRadarLegends(radarElement, hasMovements) {
...
function quadrantScrollHandler(
    ...
    const quadrantTableHeight = selectedQuadrantTable.node().getBoundingClientRect().height;
    const quadrantTableOffset = offset + quadrantTableHeight;
    const radarContainer = d3.select("#radar").node();

    // Calculate the initial left position relative to viewport
    const containerRect = radarContainer.getBoundingClientRect();
    const elementRect = radarElement.node().getBoundingClientRect();
    const initialLeftOffset = elementRect.left - containerRect.left;

    if (window.scrollY >= offset) {
        radarElement.classed("enable-transition", false);

        if (window.scrollY + margin + radarHeight >= quadrantTableOffset) {
            // Final state - absolute positioning to scroll with content
            radarElement
                .style("position", "absolute")
                .style("top", `${ quadrantTableHeight - radarHeight - margin }px`)
                .style("left", prevLeft);

            radarLegendsContainer
                .style("position", "absolute")
                .style("top", `${ quadrantTableHeight - radarHeight - margin + getScaledQuadrantHeightWithGap(scale) }px`)
                .style("left", `${ parseFloat(prevLeft.slice(0, -2)) + (getScaledQuadrantWidth(scale) / 2 - getElementWidth(radarLegendsContainer) / 2) }px`);
        } else {
            // Sticky state - fixed positioning with corrected left offset
            const containerLeft = containerRect.left;

            radarElement
                .style("position", "fixed")
                .style("top", `${ margin }px`)
                .style("left", `${ containerLeft + initialLeftOffset }px`);

            radarLegendsContainer
                .style("position", "fixed")
                .style("top", `${ margin + getScaledQuadrantHeightWithGap(scale) }px`)
                .style("left", `${ containerLeft + initialLeftOffset + (getScaledQuadrantWidth(scale) / 2 - getElementWidth(radarLegendsContainer) / 2) }px`);
        }
    } else {
        // Initial state - back to original absolute positioning
        radarElement
            .style("position", "absolute")
            .style("top", prevTop)
            .style("left", prevLeft);
        ...
        radarLegendsContainer
            .style("position", "absolute")
            .style("top", `${ parseFloat(prevTop.slice(0, -2)) + getScaledQuadrantHeightWithGap(scale) }px`)
            .style("left", `${ parseFloat(prevLeft.slice(0, -2)) + (getScaledQuadrantWidth(scale) / 2 - getElementWidth(radarLegendsContainer) / 2) }px`);
    }
}

export function stickQuadrantOnScroll() {
    if (!scrollFlag) {
        const margin = 16; // Consistent margin for sticky behavior
        const scale = getScale();
        const radarContainer = d3.select("#radar");
        ...
        const offset = radarContainer.node().offsetTop - margin; // Trigger sticky state early
        ...
        if (
            margin + radarHeight + quadrantsGap * 2 + uiConfig.legendsHeight <
