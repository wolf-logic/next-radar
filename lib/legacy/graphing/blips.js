import Blip from "@/lib/legacy/models/blip";
import Chance from "chance";
import { graphConfig } from "./config";
import { toRadian } from "@/lib/legacy/util/mathUtils";
import { renderBlipDescription } from "./components/quadrantTables";
import isEmpty from "lodash/isEmpty";
import { replaceSpaceWithHyphens, removeAllSpaces } from "@/lib/legacy/util/stringUtil";

const _ = {
    sortBy: require("lodash/sortBy"),
};

export function getRingRadius(ringIndex) {
...
function findExistingBlipCoords(ringIndex, deg) {
...
function groupBlipsBaseCoords(ringIndex) {
...
function transposeQuadrantCoords(coords, blipWidth) {
...
function createGroupBlip(blipsInRing, blipType, ring, quadrantOrder) {
    ...
    const groupBlip = new Blip(blipId, blipText, ring, blipsInRing[0].isNew(), "", "");
    groupBlip.setBlipText(blipText);
    groupBlip.setIsGroup(true);
    return groupBlip;
}

function plotGroupBlips(ringBlips, ring, quadrantOrder, parentElement, quadrantWrapper, tooltip, userSlug, radarSlug) {
    let newBlipsInRing = [],
        existingBlipsInRing = [];
    ...
    blipGroups.forEach((blipsInRing) => {
        ...
        renderBlipDescription(groupBlip, ring, quadrantWrapper, tooltip, groupBlipTooltipText, userSlug, radarSlug);
        blipsInRing.forEach(function (blip) {
            blip.setGroupIdInGraph(groupBlip.id());
            renderBlipDescription(blip, ring, quadrantWrapper, tooltip, null, userSlug, radarSlug);
        });
    });
}

export function plotRadarBlips(parentElement, rings, quadrantWrapper, tooltip, userSlug, radarSlug) {
    ...
    rings.forEach(function (ring, i) {
        ...
        if (ringBlips.length > graphConfig.maxBlipsInRings[i]) {
            plotGroupBlips(ringBlips, ring, quadrantOrder, parentElement, quadrantWrapper, tooltip, userSlug, radarSlug);
            return;
        }
        ...
        allBlipCoordsInRing.forEach(function (blipCoords, i) {
            ...
            renderBlipDescription(ringBlips[i], ring, quadrantWrapper, tooltip, null, userSlug, radarSlug);
        });
    });
}

function sortBlipCoordinates(blipCoordinates, quadrantOrder) {
    return _.sortBy(blipCoordinates, (coord) => calculateAngleFromAxis(coord, quadrantOrder));
}

function calculateAngleFromAxis(position, quadrantOrder) {
